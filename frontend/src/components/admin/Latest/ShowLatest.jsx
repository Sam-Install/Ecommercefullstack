...
const ShowLatest = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error("Error deleting latest item:", err);
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between mb-4">
        <Link to="/admin/createL/">
          <button className="bg-blue-500 px-8 py-2 rounded">Create Latest</button>
        </Link>
        <Link to="/dashboard">
            ...
            items.map((item) => (
              <tr key={item.id} className="hover:bg-gray-100">
                ...
                <td className="px-4 py-2 border space-x-2">
                  <Link to={`/admin/editlatest/${item.id}`}>
                    <button className="bg-green-500 text-white text-sm px-4 py-2 rounded">Edit</button>
                  </Link>
                  <button
